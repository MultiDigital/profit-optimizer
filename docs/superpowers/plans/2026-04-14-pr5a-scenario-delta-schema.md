...
- **Add `hr_scenario_id` column to `scenario_member_events`** so canonical-override events can be scoped to a scenario server-side (removing the client-side filter workaround in Task 6 Step 2, and fixing the cross-scenario leakage risk reviewer called out — currently mitigated by PR 5a not authoring any canonical overrides beyond the one-time migration).
- **Drop unused signature args** from `addHRScenario` (`_catalogMembers`, `_catalogEvents` left as stubs for PR 5a compat).
- **Replace `useHRPlanning` + `computeYearlyView`**: the new OR check in compute.ts is functionally correct (`scenario_member_id === memberId || member_id === memberId`) but relies on UUID uniqueness across canonical and synthetic member pools. Replace with the resolver's explicit discriminator in PR 5b or PR 6.
- **Widen `useHRPlanning` signature** to accept `Array<Member | HRScenarioMember>` + `Array<MemberEvent | ScenarioMemberEvent>` (currently union-of-arrays, forcing `as MemberEvent[]` casts at 5 call sites in `hr-planning`, `workforce-analytics`, and related pages). Two-line fix; eliminates all the downstream casts.
...
- **Resolver discriminator hardening**: `'hr_scenario_id' in m` at `resolve.ts:307` is latent-fragile — if a future `Member` schema gains an optional `hr_scenario_id`, canonical members get silently mis-classified. Switch to `(m as any).is_synthetic === true` once `is_synthetic` propagates to the TS type everywhere, or pass two typed arrays to `resolveWorkforceAtDate`.
- **`duplicateHRScenario` misses canonical-override events** — it filters by `scenario_member_id !== null`, silently skipping overrides attached via `member_id`. Update when PR 5b starts authoring canonical overrides.
- **Migration rollback story** — the PR 5a migration is destructive (deletes copy-member rows + byte-identical events). No review-list or backup table step was added (the spec suggested one). Rely on pre-migration DB backup; add a backup-table step in a later schema-level PR if risk tolerance demands it.
