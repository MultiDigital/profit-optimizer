...
- **Add Member partial-failure (CDC allocations):** in `/dashboard/workforce/page.tsx`'s `addMemberWithCdc`, the member row is created first and then `setAllocation` is called sequentially per allocation. If a mid-loop `setAllocation` throws (e.g., RLS hiccup), the user sees the "Team member added" success toast followed by a failure toast, the member exists with partial CDC rows, and the dialog stays open with the form populated — risking a duplicate-create on retry. Proper fix: a transactional RPC (`add_member_with_allocations`) or a compensating delete. Minimum mitigation: close the dialog after the first allocation failure and surface a single "Member created; some allocations failed — fix in detail page" toast.
- **`useMemberEvents` over-fetch on the list page:** the hook unconditionally joins `event_cost_center_allocations` for all CDC-typed events in the result set. The workforce list only needs event start_dates for the "Upcoming" count. Consider a `useMemberEventsList(memberIds?)` variant or an `includeAllocations: false` option in a later PR.
