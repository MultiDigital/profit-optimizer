...
- **Scenario delta schema** — PR 5 adds `member_id` to `hr_scenarios_events`, enabling canonical-member overrides. `selectScenarioData` will need to extend its output bundle to surface both canonical members AND their scenario event overrides, instead of swapping to scenario-member full-copies. Design the bundle shape change carefully to avoid breaking the PR 4 callsites.
- **Cache for `useHRScenarios`** — the "PR 3 follow-ups" note about double-fetching becomes more acute here: `useResolvedScenario` also calls `useHRScenarios` internally, so the workforce-analytics Compare tab (two calls) plus any page-level call could trigger redundant fetches. Revisit with React Query or a context-owned cache if the repeat calls cause user-visible latency.
